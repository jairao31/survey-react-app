...
//route to get all answers for all questions by a user for a survey
router.get("/getAnswerByUser/:surveyId/:uId", (req, res) => {
  const surveyID = req.params.surveyId;
  // const qID = req.params.qId;
  // const cID = req.params.cId;
  const uID = req.params.uId;
  db.query(
    "SELECT * FROM answers WHERE uId = ? AND surveyId = ?",
    [uID, surveyID],
    (err, result) => {
      if (err) {
        res.json(err);
